...
class FEM {
 public:
  ...
  double basis_function(unsigned int node, double xi_1, double xi_2,
                        double xi_3);
  std::vector<double> basis_gradient(unsigned int node, double xi_1,
                                     double xi_2, double xi_3);
  ...
  Table<2, double>
      nodeLocation;  // Table of the coordinates of nodes by global dof number
...
template <int dim>
std::vector<double> FEM<dim>::basis_gradient(unsigned int node, double xi_1,
                                             double xi_2, double xi_3) {

  std::vector<double> values(dim, 0.0);  // Store the value of the gradient of
                                         // the basis function in this variable
...
void FEM<dim>::define_boundary_conds() {
  ...
  for (uint i = 0; i < totalNodes; ++i) {
    if (std::abs(nodeLocation[i][0]) < 1.e-8) {
      boundary_values[i] =
          300 * (1 + 1 / 3 * (nodeLocation[i][1] + nodeLocation[i][2]));
    }
    if (std::abs(nodeLocation[i][0] - 0.04) < 1.e-8) {
      boundary_values[i] =
          310 * (1 + 1 / 3 * (nodeLocation[i][1] + nodeLocation[i][2]));
    }
  }
}
...
void FEM<dim>::setup_system() {
  ...
  // Fill in the Table "nodeLocations" with the x, y, and z coordinates of each
  // node by its global index
          ...
          for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < dim; j++) {
              for (unsigned int A = 0; A < dofs_per_elem; A++) {
                Jacobian[i][j] +=
                    nodeLocation[local_dof_indices[A]][i] *
                    basis_gradient(A, quad_points[q1], quad_points[q2],
                                   quad_points[q3])[j];
              }
            }
          }
            ...
            for (unsigned int B = 0; B < dofs_per_elem; B++) {
              for (unsigned int i = 0; i < dim; i++) {
                for (unsigned int j = 0; j < dim; j++) {
                  for (unsigned int I = 0; I < dim; I++) {
                    for (unsigned int J = 0; J < dim; J++) {
                      // EDIT - Define Klocal. You will need to use the inverse
                      // Jacobian ("invJacob") and "detJ"
                      Klocal[A][B] +=
                          basis_gradient(A, quad_points[q1], quad_points[q2],
                                         quad_points[q3])[i] *
                          invJacob[i][I] * kappa[I][J] *
                          basis_gradient(B, quad_points[q1], quad_points[q2],
                                         quad_points[q3])[j] *
                          invJacob[j][J] * quad_weight[q1] * quad_weight[q2] *
                          quad_weight[3] * detJ;
                    }
                  }
                }
...
void FEM<dim>::output_results() {
  ...
  std::ofstream output1("sol_b.vtk");
